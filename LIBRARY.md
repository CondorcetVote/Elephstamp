...
- [The lifecycle of a proof](#the-lifecycle-of-a-proof)
- [Stamping](#stamping)
  - [Stamp a file](#stamp-a-file)
  - [Describing what to stamp](#describing-what-to-stamp)
  - [Stamping several files at once](#stamping-several-files-at-once)
  - [Privacy nonce](#privacy-nonce)
  - [Hash algorithm](#hash-algorithm)
- [Upgrading](#upgrading)
  - [Collect the completed proof](#collect-the-completed-proof)
...
- [Reading a receipt](#reading-a-receipt)
  ...
  - [Printing the proof tree](#printing-the-proof-tree)
...
- [Configuration](#configuration)
  - [Calendars and threshold](#calendars-and-threshold)
  - [Upgrade whitelist (security)](#upgrade-whitelist-security)
  - [Customising the HTTP client](#customising-the-http-client)
...
Requires PHP 8.5+ with the `mbstring` extension. The `curl` extension is
recommended: the Symfony HTTP client then uses it for calendars, block
explorers and nodes (faster concurrent requests, HTTP/2), instead of falling
back to PHP streams.

## The lifecycle of a proof

1. **Stamp**: `stamp()` sends your file's digest to the calendar servers and
   returns a **pending** `Receipt`. Save it as an `.ots` file.
2. **Upgrade**: a few hours later, once Bitcoin has confirmed the calendars'
   transaction, `upgrade()` fetches the missing part of the proof. The receipt
   becomes **complete**.
3. **Verify**: `verify()` recomputes the proof locally and checks it against a
   Bitcoin block header.

Each step has its section below. Everything goes through one object, the
`ElephStamp` client; `new ElephStamp()` uses the public calendars and
mempool.space, see [Configuration](#configuration) for the rest.
...
`requiredCalendars` of them accept it (see
[Calendars and threshold](#calendars-and-threshold)).
...
Only calendars allowed by the [upgrade whitelist](#upgrade-whitelist-security)
are contacted.
...
#### One good attestation is enough

A proof usually holds one branch per calendar, and they are independent: a
branch whose block does not commit to the proof does not undo one that does.
So a mismatch only makes the verdict `Failed` when no other attestation is
verified (a file mismatch always does). Mismatches stay visible whatever the
verdict, and are worth surfacing: one of the calendars handed out something
wrong.
...
#### What an explorer is trusted for
